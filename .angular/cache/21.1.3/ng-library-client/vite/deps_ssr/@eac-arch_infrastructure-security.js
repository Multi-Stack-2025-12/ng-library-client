...
// node_modules/@eac-arch/infrastructure-security/fesm2022/eac-arch-infrastructure-security.mjs
...
var OAuthService = class _OAuthService extends AuthConfig {
  ...
  revokeTokenAndLogout(customParameters = {}, ignoreCorsIssues = false) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (ignoreCorsIssues) {
        revokeAccessToken = revokeAccessToken.pipe((0, import_operators.catchError)((err) => {
          if (err.status === 0) {
            return (0, import_rxjs.of)(null);
          }
          return (0, import_rxjs.throwError)(err);
        }));
        revokeRefreshToken = revokeRefreshToken.pipe((0, import_operators.catchError)((err) => {
          if (err.status === 0) {
            return (0, import_rxjs.of)(null);
          }
          return (0, import_rxjs.throwError)(err);
...
var DefaultOAuthInterceptor = class _DefaultOAuthInterceptor {
  ...
  intercept(req, next) {
    ...
    if (!sendAccessToken) {
      return next.handle(req).pipe((0, import_operators.catchError)((err) => this.errorHandler.handleError(err)));
    }
...
function extractRoles(claims) {
  ...
  if (Array.isArray(claims["roles"])) for (const r of claims["roles"]) out.add(String(r).toLowerCase());
  if (Array.isArray(claims["groups"])) for (const g of claims["groups"]) out.add(String(g).toLowerCase());
  const realmAccess = claims["realm_access"];
  if (Array.isArray(realmAccess?.roles)) for (const r of realmAccess.roles) out.add(String(r).toLowerCase());
  const ra = claims["resource_access"];
  if (ra) for (const k of Object.keys(ra)) ra[k]?.roles?.forEach((r) => out.add(String(r).toLowerCase()));
...
function mapClaimsToUserinfo(claims) {
  const first2 = claims["given_name"] ?? claims["givenName"] ?? claims["firstName"] ?? null;
  const last = claims["family_name"] ?? claims["familyName"] ?? claims["lastName"] ?? null;
  const display = (claims["name"] ?? [first2, last].filter(Boolean).join(" ")) || claims["preferred_username"] || claims["email"] || claims["sub"] || "";
  return {
...
var GraphProfileLoader = class {
  oauth;
  getIdpConfig;
  accessToken = signal(null, ...ngDevMode ? [{
    debugName: "accessToken"
  }] : []);
  accessTokenExp = 0;
  ...
  isAzure() {
    const issuer = this.oauth?.issuer || this.getIdpConfig()?.ISSUER || "";
    return /login\.microsoftonline\.com/i.test(issuer) || /sts\.windows\.net/i.test(issuer);
  }
  async loadProfile() {
    const token = await this.ensureToken();
    if (!token) return null;
    const resp = await fetch("https://graph.microsoft.com/v1.0/me?$select=displayName,givenName,surname,mail,userPrincipalName,id", {
      headers: {
  ...
  async exchangeRefreshToken() {
    ...
    if (at) {
      ...
      this.accessTokenExp = Date.now() + Math.max(0, (expiresIn - 60) * 1e3);
      try {
...
var ProfileResolver = class {
  ...
  constructor(userinfo, rolesSignal, oauth, graphLoader, getPayload) {
    ...
    this.profile = computed(() => this.buildProfile(), ...ngDevMode ? [{
      debugName: "profile"
    }] : []);
  }
  async loadFromIdp() {
    ...
    try {
      ...
      let userinfo = __spreadValues(__spreadValues({}, mapClaimsToUserinfo(claims)), info);
      if (this.graphLoader.isAzure()) {
        const graphInfo = await this.graphLoader.loadProfile();
        if (graphInfo) userinfo = __spreadValues(__spreadValues({}, userinfo), graphInfo);
      }
      userinfo["roles"] = Array.isArray(info?.["roles"]) && info["roles"].length ? info["roles"] : roles;
  ...
  async loadFallback(roles) {
    if (this.graphLoader.isAzure()) {
      try {
        const graphInfo = await this.graphLoader.loadProfile();
        if (graphInfo) {
          const claims2 = this.resolveClaims();
          this.userinfo.set(__spreadProps(__spreadValues(__spreadValues({}, mapClaimsToUserinfo(claims2)), graphInfo), {
            roles
          }));
          return;
        }
    ...
    const claims = this.resolveClaims();
    this.userinfo.set(__spreadProps(__spreadValues({}, mapClaimsToUserinfo(claims)), {
      roles
    }));
  }
  resolveClaims() {
  ...
  buildProfile() {
    ...
    if (!c) return null;
    const first2 = c["given_name"] ?? c["givenName"] ?? c["firstName"] ?? null;
    const last = c["family_name"] ?? c["familyName"] ?? c["lastName"] ?? null;
    const display = (c["name"] ?? [first2, last].filter(Boolean).join(" ")) || c["preferred_username"] || c["email"] || c["sub"] || "";
    return {
      sub: c["sub"] ?? null,
      displayName: display,
      firstName: first2,
...
var TokenManager = class {
  ...
  scheduleAutoRefresh(oauth) {
    ...
    const delay2 = Math.max(0, expMs - Date.now() - this.REFRESH_BEFORE_MS);
    this.refreshTimer = setTimeout(() => {
      oauth.refreshToken().then(() => this.scheduleAutoRefresh(oauth)).catch(() => this.onExpired());
    }, delay2);
  }
  clearSchedule() {
...
var SecurityService = class {
  ...
  constructor(isBrowser, oauth, router, getIdpConfig, destroyRef) {
    ...
    this.oauthEvt = signal(null, ...ngDevMode ? [{
      debugName: "oauthEvt"
    }] : []);
    this.accessToken = computed(() => (this.oauthEvt(), this.oauth.getAccessToken() ?? null), ...ngDevMode ? [{
      debugName: "accessToken"
    }] : []);
    this.payload = computed(() => decodeJwtPayload(this.accessToken()), ...ngDevMode ? [{
      debugName: "payload"
    }] : []);
    this.isLoggedIn = computed(() => (this.oauthEvt(), this.oauth.hasValidAccessToken()), ...ngDevMode ? [{
      debugName: "isLoggedIn"
    }] : []);
    this.scopesSignal = computed(() => extractScopes(this.payload() ?? {}), ...ngDevMode ? [{
      debugName: "scopesSignal"
    }] : []);
    this.rolesSignal = computed(() => {
      const claims = this.payload() ?? this.oauth.getIdentityClaims() ?? {};
      return extractRoles(claims);
    }, ...ngDevMode ? [{
      debugName: "rolesSignal"
    }] : []);
    const userinfo = signal(null, ...ngDevMode ? [{
      debugName: "userinfo"
    }] : []);
  ...
  hasAnyRole(required) {
    return hasAny(this.rolesSignal(), required);
  }
  hasAllRoles(required) {
    return hasAll(this.rolesSignal(), required);
  }
  hasAnyScope(required) {
    return hasAny(this.scopesSignal(), required);
  }
  login(target) {
    const rel = target || this.router.url || "/";
    this.oauth.initLoginFlow(stateEncode({
      rel,
      t: Date.now()
    }));
  }
  logout() {
    const idp = this.getIdpConfig();
    this.oauth.logOut({
      post_logout_redirect_uri: idp?.POST_LOGOUT_REDIRECT_URI
    });
  }
  // -- Private --
  buildAuthConfig(idp) {
  ...
  subscribeToOAuthEvents() {
    ...
    this.oauth.events.pipe((0, import_rxjs2.startWith)(null)).subscribe((e) => {
      evtSignal.set(e);
      if (!e) return;
      if (e instanceof OAuthErrorEvent) {
...
function provideSecurity() {
  return makeEnvironmentProviders([provideOAuthClient(), {
...
var authzGuard = (route) => {
  ...
  const ok = authService.hasAnyRole(any) && authService.hasAllRoles(all) && authService.hasAnyScope(scopesAny);
...
var authInterceptor = (req, next) => {
  ...
  const isSecure = keys.map((key) => config.get(key)).filter((url) => !!url).some((url) => requestUrl.startsWith(url));
  if (isSecure) {
...
var OidcCallback = class _OidcCallback {
  ...
  static ɵfac = function OidcCallback_Factory(__ngFactoryType__) {
    return new (__ngFactoryType__ || _OidcCallback)();
  };
  static ɵcmp = ɵɵdefineComponent({
    type: _OidcCallback,
    selectors: [["eac-oidc-callback"]],
    decls: 0,
    vars: 0,
    template: function OidcCallback_Template(rf, ctx) {
    },
    encapsulation: 2
  });
};
(() => {
  (typeof ngDevMode === "undefined" || ngDevMode) && setClassMetadata(OidcCallback, [{
    type: Component,
    args: [{
...
export {
  ...
  safeRel,
  stateDecode,
  stateDecodeFromUrlParam,
  stateEncode
};
//# sourceMappingURL=@eac-arch_infrastructure-security.js.map
